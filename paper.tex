...
また，骨格推定に関する先行研究を通じて本研究の基礎を築かれた須田悠介氏，実験や議論を通じて多大なる協力をいただいた宮脇拓真氏をはじめとする研究室の皆様，同期学生の皆様，並びに私を暖かく見守っていただいた両親をはじめとする周囲のすべての皆様に深く感謝致します．
...
\setcounter{page}{1}   %消さないこと
       ...
       複数人物の行動理解には，カメラ画像が用いられることが多い．カメラ画像は情報量が多いため，簡単に行動の解析ができる．一方で，公共空間での撮影ではプライバシーの問題が発生する．不特定多数が利用する公共空間において，カメラ画像による計測を行うと，顔などの重要度の高い個人情報を収集することになる．カメラ画像を用いて歩行者計測を行う場合は，個人が特定されない形での情報処理や利用用途の正しい告知などが求められ，計測を行うハードルが高い．
...
本研究で実装する歩行者計測システムに関わる先行研究を紹介する．
       ...
       小林研究室では，以前より足元の高さに設置した2D-LiDARを用いた歩行者追跡が行われている\cite{hasan2021person}\cite{h_b}\cite{h_m}．先行研究では，クラスタリングを用いた検出とカルマンフィルタを用いた追跡によって，歩行者追跡を実現していた．本節では先行研究の概要について説明する．
              ...
              2D-LiDARによって取得された人物の足は，図\ref{fig:dis}のように描画される．本稿ではこれを距離画像と呼ぶ．距離画像に対してクラスタリングを行うと，正しく人を検出することができない場合がある．右足と左足が離れている場合に2人として検出してしまう問題や，複数人が近接する場合に1人として検出してしまう問題が発生する．そこで先行研究では，過去フレームの画像を重ね合わせた画像(図\ref{fig:mhi})をクラスタリングに用いている．本稿ではこれを時系列画像と呼ぶ．時系列画像は現在の足の位置だけでなく過去の動きを表現できるため，前述の問題を解消できる．
              ...
              処理速度も課題となっていた．計測人数に比例して処理時間が大きくなっていくため，複数の歩行者がいる場合，リアルタイムでの計測は難しかった．時系列画像作成，クラスタリング，カルマンフィルタの適用を並列で行うことで高速化を目指したものの，クラスタリングがボトルネックとなり高速化は実現できなかった．
       ...
       カメラ画像に対する複数物体追跡は，Multi Object Tracking（MOT）と総称され，様々な手法が提案されている．本研究では，その中でも極めて高い性能を発揮することで知られるByteTrack\cite{ByteTrack}のアルゴリズムを，2D-LiDAR画像に適用することを考えた．本節ではByteTrackの概要を紹介する．
              ...
              \begin{center}
                     ...
                     \caption[人物の検出と追跡(一般的なアルゴリズムの場合)]{人物の検出と追跡(一般的なアルゴリズムの場合)\newline (\cite{ByteTrack}図2より)}
                     \label{fig:com}
              \end{center} 
              \end{figure}
       ...
       \section{骨格推定}
       小林研究室では，2D-LiDARを用いた足元計測データから，歩行者の全身骨格を推定する研究が行われてきた\cite{suda2023}．本節では先行研究における3次元骨格推定の概要について説明する(図\ref{fig:sudasan})．
       ...
       \begin{center}
              \centering
              \includegraphics[width=130mm]{sudasan.png}
       ...
       \end{figure}
              ...
              \subsection{学習データの作成}
              先行研究では，学習データとして用いる骨格情報の取得のためにRGB-DカメラであるKinectを用いている．足元に設置した2D-LiDARで直進歩行を記録すると同時に，正面に設置したKinectで全身の骨格情報をあわせて取得して学習データとする(図\ref{fig:suda_learn})．
              
              取得された2D-LiDARの点群データは，歩行者検出・追跡と同様に時系列画像として書き出す．この時系列画像から人物の領域を重心を中心に100×100ピクセルで切り出したものを，ネットワークへの入力とする．学習データは204,466枚，テストデータは16,572枚であり，歩行者の正面・背後・左右から記録した4方向の直進歩行のみが含まれている．
...
本システムは，計測データの取得と結果の描画を行うPC(以下計測用マシン)と，深層学習モデルの推論などの重い計算処理を行うGPUを搭載したPC(以下計算用マシン)の2台の連携によって動作する．

全体の処理の流れは以下の通りである．
\begin{enumerate}
       \item 計測用マシンが2D-LiDARから距離データを取得
       \item 取得した距離データを計算用マシンへ送信
       \item 計算用マシンにて受信した距離データを直交座標系へ変換・MHIを作成
       \item ByteTrackによる追跡処理と骨格推定モデルによる推定処理を並列に実行
       \item 計算用マシンのメインスレッドが各処理結果を計測用マシンへ逐次送信
       \item 受信した結果を用いて3次元描画
\end{enumerate}

この構成により，計算負荷の高い処理を計算用マシンに集約し，リアルタイムでの計測と描画を実現している．本システムの構成を図\ref{fig:overall_system_flow}に示す．本章では，各処理の詳細について述べる．

\begin{figure}[H]
    \centering
    \includegraphics[width=120mm]{system_flow_3.png}
    \caption{システム構成図}
    \label{fig:overall_system_flow}
\end{figure}
       ...
       \section{2D-LiDARによる計測}
              ...
              測距精度 & 0.1〜10m:±30mm， 10〜30m:±50mm \\ \hline
              走査角度 & 270度\\ \hline
              ...
              \subsection{距離データの取得}
              2D-LiDARより取得されるのは，計測平面における周囲の物体との距離データである．周囲270度を0.25度ずつ走査するため，1080点の距離データが取得される．本システムでは，取得した距離データをそのまま計算用マシンへ送信している．手元のマシンで座標変換して画像化する場合，2次元画像の送受信が必要となり処理速度が低下するため，距離データをそのままテキストデータとして送信することでシステムの高速化を図った．
       ...
       メインスレッドでは，距離データの受信と時系列画像の作成，各処理より得られた結果の送信を行う．各スレッドでの処理の結果はグローバル変数へ格納され，メインスレッドはそれらの更新にかかわらず一定のタイミングで計測用マシンへの送信を行う．これによって，各処理の時間に影響を受けることなくシステム全体が動作する．
       ...
       本研究では，カメラ画像に対する複数物体追跡手法を2D-LiDARによる計測に導入することで，従来の手法の課題解消を目指した．2D-LiDAR画像は情報量が極めて少ないため，追跡対象の見た目の特徴を追跡の手がかりとしないにも関わらず高い性能を発揮しているByteTrackを採用している．ByteTrackによる複数物体追跡は，YOLOXによる検出とBYTEアルゴリズムによる追跡によって実現される．それらの処理は計算用マシンの検出・追跡用スレッドで実行される．
              ...
              \subsection{YOLOXによる検出およびモデル構築}
              本研究においても，従来のByteTrackと同様にYOLOXを用いた人物の検出を目指す．しかし，本来のYOLOXはカメラ画像中の一般物体を対象としており，そのままでは2D-LiDAR計測データより作成した時系列画像において人物を検出することは難しい．そこで本システムの検出手法として利用するために，新たな転移学習を行う必要があった．

              ベースモデルには，YOLOXシリーズの中で最もパラメータ数が多く高精度なYOLOX-Xモデルを採用した．学習データセットには，実際に歩行者の足元の様子を2D-LiDARで計測し，時系列画像を作成してアノテーションを行ったものを用いている．データの総数は2499枚である．また，モデルの汎化性能を正しく評価するために，評価用データセット650枚には，学習データには一切含まれていない未知の被験者の歩行データを使用した．

              上記のデータセットを用いて50エポックの学習を行った．評価用データに対する平均適合率（Average Precision: AP）の結果を表\ref{tab:yolox_ap}に示す．IoU=0.50:0.95の総合指標において0.815となり，IoU=0.50およびIoU=0.75においては0.990という高い数値を記録した．歩行者追跡システムにおける検出器として十分な性能を得ることができた．

              \begin{table}[H]
                     \centering
                     \caption{構築した検出モデルの性能評価}
                     \begin{tabular}{|l||c|} \hline
                            Metrics & Value \\ \hhline{|=||=|}
                            $AP$ (IoU=0.50:0.95) & 0.815 \\ \hline
                            $AP_{50}$ (IoU=0.50) & 0.990 \\ \hline
                            $AP_{75}$ (IoU=0.75) & 0.990 \\ \hline
                     \end{tabular}
                     \label{tab:yolox_ap}
              \end{table}
              ...
              \subsection{追跡結果の送信}
              YOLOXによる検出，BYTEアルゴリズムによる追跡を経て得られた追跡結果は，追跡対象ごとにIDと座標情報がセットとなり出力される．これらはグローバル変数に格納され，メインスレッドによって計測用マシンへ送り返される．
       ...
       \begin{figure}[H]
              \centering
              \includegraphics[width=100mm]{g_crop.png}
       ...
       このモデルは，ImageNetで事前学習済みのResNet-18をベースとした回帰モデルであり，入力されたMHIから25点の3次元骨格座標を直接推定する．学習データは合計204,466枚（テストデータ16,572枚）であり，現在は歩行者の正面・背後・左右の4方向の直進歩行に限定して学習が行われている．
       学習時には，平均二乗誤差(MSE)に基づく損失関数に加え，手足の末端など誤差が大きくなりやすい部位に対して動的に重み付けを行う独自の損失関数を用いることで，歩行動作における手足の振りをより正確に再現できるよう学習されている．
       推定結果は25点の3次元座標として出力される．それらの結果は検出・追跡と同様にグローバル変数に格納され，メインスレッドによって計測用マシンへ送り返される．
       ...
       本節では，計算用マシンより送り返された各結果の処理方法について述べる．
       
       計測用マシンは，計算用マシンから送られてくる追跡結果および骨格推定結果を受信する．この際，それぞれの結果には計測時に付与されたタイムスタンプが含まれている．処理時間の違いにより，追跡結果と骨格推定結果は異なるタイミングで到着する可能性があるため，描画を行う前にタイムスタンプの照合を行う．同一のタイムスタンプを持つデータが揃った段階で初めて，それらを統合して描画処理へと回す．これにより，時間のズレたデータが誤って紐付けられることを防ぎ，正しい結果表示が行われる．

       本システムでは，2次元平面での検出・追跡に関する結果表示と，3次元空間での骨格推定を含めた結果表示を実装した．
 ...
 続く4.2節では，骨格推定を含めた統合システムの評価を行う．実際に複数人が歩行する環境においてシステムを稼働させ，検出・追跡の結果に基づいて骨格推定が行われているか確認し，各状況における成功・失敗の要因について考察を行う．
       ...
       本節では，歩行者の検出・追跡に関する性能改善について評価する．複数人の歩行に対して，今回提案するByteTrackベースの手法と従来の手法のそれぞれで追跡を行う．
              \subsection{実験の環境}
                     ...
                     計測は1台の2D-LiDARによって行われる．ByteTrackベースの提案手法では，リアルタイムに追跡を行った．また，従来の手法でも同じ歩行に対して追跡を適用する．2D-LiDARから受け取る距離データをテキストファイルとして書き出し，従来の手法の追跡プログラムで読み込むことで，従来の手法でも追跡を行った．
              ...
              \subsection{評価指標}
                     ...
                     MOTAには，追跡の持続性を評価することができないという弱点があった．IDF1\cite{ristani2016performance}は，IDFP (ID False Positive)やIDFN (ID False Negative)といったIDを基準とした数値を用いることで，MOTAの弱点を解消した評価基準である．定義を式\ref{eq:idf1}に示す．IDFPは，従来のFPに加えて，IDの付与を誤っている場合もミスとしてカウントする．同様にIDFNは，従来のFNに加えて，IDの付与を誤っている場合もミスとしてカウントする．IDTPはすべての検出や正解からIDFPもしくはIDFNを引くことで求められ，正しく検出されなおかつIDの付与も合っている検出の数を示す．これらを用いて計算されるIDF1は，正しいIDを多くのフレームで付与できているほど高い数値となる．
        ...
        本節では，骨格推定を含めた統合システムの評価を行う．
              ...
              まず，骨格推定モデルを学習する際に学習データとして用いた，シンプルな直進歩行に対して統合システムを実行して骨格を推定する．行う歩行は，2D-LiDARに向かって直進，2D-LiDARから離れる方向に直進，2D-LiDARの前を横断の3種類である．
                     ...
                     \begin{figure}[H]
                            ...
                            \caption[2D-LiDARに向かって直進する場合の推定]{2D-LiDARに向かって直進する場合の推定}
                            \label{fig:front}
                     \end{figure}
                     ...
                     2D-LiDARから離れていく方向に直進する歩行を計測すると，多くの場合においては正確な骨格が推定できた(図\ref{fig:back})．
                     
                     \begin{figure}[H]
                            \centering
                            \includegraphics[width=115mm]{back.png}
                            \caption[2D-LiDARから離れる方向に直進する場合の良好な推定]{2D-LiDARから離れる方向に直進する場合の良好な推定}
                            \label{fig:back}
                     \end{figure}

                     一方で，いくつかの課題も見られた．最も多かった誤りは，進行方向に対して身体の向きが逆に推定されてしまう現象である(図\ref{fig:reverse})．

                     \begin{figure}[H]
                            \centering
                            \includegraphics[width=115mm]{reverse.png}
                            \caption[進行方向と逆向きに推定された様子]{進行方向と逆向きに推定された様子}
                            \label{fig:reverse}
                     \end{figure}
                     
                     また，明確に崩れた骨格が推定されるフレームも存在した(図\ref{fig:back_miss})．学習データにおける2D-LiDARから離れる歩行は，多くが画像をはみ出る形で足が描画されていた．本システムでは重心の再計算により画像をはみ出すことは極めて少なくなっているため，学習時と入力データの傾向が異なり，崩れた骨格が見られたのではないかと考える．
                     ...
                     2D-LiDARの前を横断する場合は，左右どちらの向きであってもおおよそ正しい推定ができた(図\ref{fig:right_good})．明確に崩れた骨格が推定されるフレームも少なかった．

                     \begin{figure}[H]
                            \centering
                            \includegraphics[width=115mm]{right_good.png}
                            \caption[2D-LiDARの前を横断する場合の良好な推定]{2D-LiDARの前を横断する場合の良好な推定}
                            \label{fig:right_good}
                     \end{figure}
                     
                     一方で，足の振りや腕の振りが実際よりも大きく推定されるフレームが多かった(図\ref{fig:right})．これは，切り抜いた時系列画像中で足の動きがはみ出してしまっていたことに起因すると考えられる．
              ...
              \subsection{実環境に近い歩行における骨格推定}
               続いて，実際に計測を行う環境に近い複数人の歩行に対して，統合システムを実行して骨格を推定した．
複数人が歩行する場合でも，それぞれの骨格をリアルタイムで推定し，追跡結果も含めて3次元点群として出力できることを確認した．
              ...
              \begin{figure}[H]
                     \centering
                     \includegraphics[width=150mm]{unit_across.png}
                     \caption[2D-LiDARの前を横切る歩行]{2D-LiDARの前を横切る歩行}
                     \label{fig:unit_across}
              \end{figure}
              

              また，2D-LiDARの正面で直進歩行を行う場合にも，正しい推定を行える場合が多かった(図\ref{fig:unit_front})．
              
              \begin{figure}[H]
                     \centering
                     \includegraphics[width=150mm]{unit_front.png}
                     \caption[2D-LiDARの正面で直進する歩行]{2D-LiDARの正面で直進する歩行}
                     \label{fig:unit_front}
              \end{figure}


              一方で，2D-LiDARの正面ではない位置で直進歩行する場合は，推定精度がわずかに低下した．図\ref{fig:naname}の水色の骨格がその例である．
              歩行の向きとしては，前節の2D-LiDARに向かって直進する場合に近い．しかし，2D-LiDARから見て斜め前での歩行であったため，時系列画像に残る足の形が正面から見た形と変化してしまい，推定精度にわずかに影響したと考えられる．

              

               また，直進ではなく曲線的な動きをメインとする歩行においては，ほとんどの場合で崩れた骨格となってしまった．また，推定できたフレームであっても，歩行方向と異なる向きでの推定が多かった．図\ref{fig:naname}の紫色の骨格がその例である．
これは，現在の学習データには斜めの歩行や曲線的な歩行が含まれていないためであると考えられる．

              \begin{figure}[H]
                     \centering
                     \includegraphics[width=150mm]{naname.png}
                     \caption[推定がうまくいかない例]{推定がうまくいかない例}
                     \label{fig:naname}
              \end{figure}
...
\chapter{おわりに}
       ...
       \subsection{多様な歩行動作への対応}
        第4章の実験結果において，直進以外の歩行や複雑な軌道を描く歩行に対して，骨格推定の精度が低下する傾向が見られた．これは，現在の骨格推定モデルの学習データが，単純な直進歩行のみで構成されていることによると考えられる．
