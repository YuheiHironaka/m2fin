...
本研究で実装する歩行者計測システムに関わる先行研究を紹介する．
       ...
       \section{骨格推定}
       小林研究室では，2D-LiDARを用いた足下計測データから，歩行者の全身骨格を推定する研究が行われてきた\cite{s_b}．本節では先行研究における3次元骨格推定の概要について説明する．
...
本システムは，計測データの取得と結果の描画を行うPCと，深層学習モデルの推論などの重い計算処理を行うGPUを搭載したPC(以下GPUマシン)の2台の連携によって動作する．

全体の処理の流れは以下の通りである．
\begin{enumerate}
       \item 手元のPCが2D-LiDARから距離データを取得する．
       \item 取得した距離データを，そのままGPUマシンへ送信する．
       \item GPUマシンにて，受信した距離データを直交座標系へ変換し，MHIを作成する．
       \item 作成されたMHIを用いて，ByteTrackによる追跡処理と骨格推定モデルによる推定処理を，それぞれ独立したスレッドで並列に実行する．
       \item GPUマシンのメインスレッドが各処理結果を統合し，手元のPCへ逐次送信する．
       \item 手元のPCにて，受信した結果を用いて3次元描画を行う．
\end{enumerate}

この構成により，計算負荷の高い処理をGPUマシンに集約し，リアルタイムでの計測と描画を実現している．本章では，各処理の詳細について述べる．
       ...
       \section{2D-LiDARによる計測}
              ...
              2D-LiDARは，足首を計測できる高さとして床から約20cmの位置に設置する．
              ...
              \subsection{距離データの取得}
              2D-LiDARより取得されるのは，計測平面における周囲の物体との距離データである．周囲270度を0.25度ずつ走査するため，1080点の距離データが取得される．本システムでは，取得した距離データをそのまま計算用のGPUマシンへ送信している．手元のマシンで座標変換して画像化する場合，2次元画像の送受信が必要となり処理速度が低下するため，距離データをそのままテキストデータとして送信することでシステムの高速化を図った．


       \section{2D-LiDARデータの画像処理}

             \subsection{2次元画像の作成}
              GPUマシンでは，受信した距離データを2次元画像に変換する．取得した距離データのラジアン角度を求め，直交座標系に変形してプロットする．このように描画された2次元画像を本稿では距離画像と呼ぶ．また，現在の1フレームの距離画像を，本稿では現在画像と呼ぶ．
       ...
       \section{検出・追跡と骨格推定の適用}

       作成した時系列画像に対して，検出・追跡と骨格推定を行う．それぞれの処理にかかる時間が異なることから，本システムではマルチスレッドにて各処理を並列して行う．メインスレッドに並列して，検出・追跡用スレッドと骨格推定用スレッドが動作する．
       ...
       本研究では，カメラ画像に対する複数物体追跡手法を2D-LiDARによる計測に導入することで，従来の手法の課題解消を目指した．2D-LiDAR画像は情報量が極めて少ないため，追跡対象の見た目の特徴を追跡の手がかりとしないにも関わらず高い性能を発揮しているByteTrackを採用している．ByteTrackによる複数物体追跡は，YOLOXによる検出とBYTEアルゴリズムによる追跡によって実現される．それらの処理はGPUマシンの検出・追跡用スレッドで実行される．
              ...
              実際に歩行者の足元の様子を2D-LiDARで計測し，時系列画像を作成してアノテーションを行うことで学習データとした．2499枚の学習データ，650枚のテストデータを用いて学習を行った．

              %学習ほんとはもっと詳しく書いたほうがいいよね
       ...
       本節では，骨格推定用スレッドでの処理について述べる．

       
       骨格推定を行うために，まずは計測域全体の時系列画像から歩行者の部分のみを切り出す必要がある．検出・追跡によって得られたバウンディングボックスの位置情報を元に，骨格推定モデルに合わせて100×100ピクセルの画像を切り出す．歩行者の動きによってはボックスの位置がズレてはみ出すことがあるため，一度大きな領域の画像へ広げ，重心計算を行うことで正確に足元を切り出す．図において，黄色が追跡によって得られたバウンディングボックスである．これを白色の領域に広げて重心を計算し，青色の領域を切り出している．
       ...
       得られた時系列画像に対して，先行研究で得られたモデルを用いて骨格推定を行う．推定結果は25点の3次元座標として出力される．それらの結果は検出・追跡と同様にグローバル変数に格納され，メインスレッドによって手元のPCへ送り返される．
       ...
       本節では，GPUマシンより送り返された各結果の処理方法について述べる．本システムでは，2次元平面での検出・追跡に関する結果表示と，3次元空間での骨格推定を含めた結果表示を実装した．


              \subsection{2次元平面での結果表示}

              2D-LiDARによって計測された点群を2次元平面に描画し，その上に検出・追跡結果を描画することで追跡の様子が可視化される．
              ...
              まず，2D-LiDARから得られる点群データを，3次元空間内において平面として描画する．次に，推定された骨格を描画していく．この際，追跡結果の座標情報に基づいて，骨格情報を描画する座標を決定する．足首を示す骨格点と2D-LiDAR点群のZ座標を揃えることで，実際に点群上を骨格が歩いているような表示を実現した．
              ...
              \subsection{崩れた骨格の補完}
              現在の骨格推定の精度はまだ改善の余地があり，明確に崩れた骨格が推定される場合もある．
...
\chapter{実験と考察}
 ...
 続く4.2節では，骨格推定を含めた統合システムの評価を行う．実際に複数人が歩行する環境においてシステムを稼働させ，検出・追跡の結果に基づいて骨格推定が行われているか確認し，各状況における成功・失敗の要因について考察を行う．
       ...
       本節では，歩行者の検出・追跡に関する性能改善について評価する．複数人の歩行に対して，今回提案するByteTrackベースの手法と従来の手法のそれぞれで追跡を行う．
              ...
              2人による歩行，4人による歩行のそれぞれに対して上記の評価指標を計算した．結果を表に示す．
              ...
              どちらの歩行においても，MOTAはクラスタリングとカルマンフィルタを用いる従来の手法のほうが僅かに高くなった．クラスタリングによる人物検出は，点群の密集を人物であると認識しているため，遮蔽などで足の形が通常と異なる場合にも検出性能が落ちない．一方で，ByteTrackを用いた提案手法は，YOLOXによって学習された足の形を検出しているため，遮蔽などで理想的な足の形がとれない場合などに検出に失敗する場合が多く，MOTAが伸びなかったと考える．

              %従来は検出できてるけど提案だとできてない図

              一方で，IDF1はByteTrackを用いた提案手法のほうが大幅に高くなった．これは，人物同士が接近したり遮蔽が発生した際に，これまでと違うIDを付与してしまうIDスイッチが全く発生しなかったためであると考えられる．カルマンフィルタを用いて追跡を行う従来の手法では，多くの場面でIDスイッチが発生しており，持続性の高い追跡にはなっていない．

              %従来だとスイッチしてるけど提案だとしてない図

              ByteTrackを用いた提案手法は、どちらの歩行においても同じIDを付与し続けることができた．
       ...
       本節では，骨格推定を含めた統合システムの評価を行う.
              ...
              まず，骨格推定モデルを学習する際に学習データとして用いた，シンプルな直進歩行に対して統合システムを実行して骨格を推定する．行う歩行は，2D-LiDARに向かって直進，2D-LiDARから離れる方向に直進，2D-LiDARの前を横断の3種類である．
                     ...
                     2D-LiDARに向かって直進する歩行を計測すると，非常に正確な骨格が推定できた．実際の歩行の様子と同じように，足の動きや腕の振りが再現されている．
                     ...
                     2D-LiDARから離れていく方向に直進する歩行を計測すると，多くの場合においては正確な骨格が推定できた．一方で，明確に崩れた骨格が推定されるフレームもいくつか存在した．学習データにおける2D-LiDARから離れる歩行は，多くが画像をはみ出る形で足が描画されていた．本システムでは重心の再計算により，画像をはみ出すことは極めて少なくなっている．そのために，2D-LiDARから離れる歩行では崩れた骨格が見られたのではないかと考える．
                     ...
                     2D-LiDARの前を横断する場合は，左右どちらの向きであってもおおよそ正しい推定ができた．明確に崩れた骨格が推定されるフレームも少なかった．一方で，足の振りや腕の振りが実際よりも大きく推定されるフレームが多かった．これは，切り抜いた時系列画像中で足の動きがはみ出してしまっていたことに起因すると考えられる．
              ...
              最も正しく骨格を推定できたのは，2D-LiDARの前を横切るような歩行であった．これは，前節で述べた2D-LiDARの前を横断する場合と全く同じ状況であり，学習データに極めて近い計測ができていたためであると考えられる．
              %横切る様子にフィーチャーした図
              ...
              また，直進ではなく曲線的な動きをメインとする歩行においても，推定の精度が下がってしまった．これは，学習データに直線的な歩行しかないためであると考えられる．
