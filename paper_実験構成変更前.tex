...
\chapter{システムの概要}
本研究では，2D-LiDARで計測した複数歩行者の検出・追跡と骨格推定をリアルタイムで行う，統合的なシステムを実現した．本章ではシステムの概要を示す．
       ...
       \section{人物の検出と追跡}
       本研究では，カメラ画像に対する複数物体追跡手法を2D-LiDARによる計測に導入することで，従来の手法の課題解消を目指した．2D-LiDAR画像は情報量が極めて少ないため，追跡対象の見た目の特徴を追跡の手がかりとしないにも関わらず高い性能を発揮しているByteTrackを採用している．ByteTrackによる複数物体追跡は，YOLOXによる検出とBYTEアルゴリズムによる追跡によって実現される．
              ...
              実際に歩行者の足元の様子を2D-LiDARで計測し，時系列画像を作成してアノテーションを行うことで学習データとした．〇〇枚の学習データ，〇〇枚のテストデータを用いて学習を行った．
              ...
              追跡におけるハイパーパラメータは，本論文における実験では以下のように設定している．
       ...
       本システムでは，前節までに述べた歩行者の検出・追跡のフローと，これまでに研究されてきた2D-LiDAR画像からの全身骨格推定の融合を目指した．
       
       骨格推定を行うために，まずは計測域全体の時系列画像から歩行者の部分のみを切り出す必要がある．検出・追跡によって与えられたバウンディングボックスの位置情報を元に，骨格推定モデルに合わせて100×100ピクセルの画像を切り出す．歩行者の動きによってはボックスの位置がズレてはみ出すことがあるため，200×200ピクセルで切り出して重心計算を行いズレを修正する．

       得られた時系列画像に対して，先行研究で得られたモデルを用いて骨格推定を行う．
...
\chapter{実験}
 本研究で提案するシステムの有効性を検証するために実験を行う．本章における実験は，大きく2つのフェーズに分かれる．
 
 まず前半では，歩行者の検出・追跡に関する性能改善について評価する．ここでは，MOT (Multi Object Tracking) 分野で一般的に用いられる評価指標を用いて，ByteTrackベースの提案手法と従来の手法の性能を比較する．
 
 後半では，骨格推定を含めた統合システムの評価を行う．実際に複数人が歩行する環境においてシステムを稼働させ，検出・追跡の結果に基づいて骨格推定が行われているか，またその結果が統合的に可視化されているかを確認する．
       ...
       本節では，歩行者の検出・追跡に関する性能改善について評価する．複数人の歩行に対して，今回提案するByteTrackベースの手法と従来の手法のそれぞれで追跡を行う．
              ...
              \begin{table}[H]
                     \centering
                     \caption{性能評価(歩行人数：２人)}
                     
                     \begin{tabular}{|l||r|r|r||r|r|} \hline
                            & FP & FN & IDsw & MOTA & IDF1\\ \hhline{|=|=|=|=|=|=|}
                            クラスタリング＋カルマンフィルタ & 10 & 114 & 6 & 0.944 & 0.962\\ \hline
                            ByteTrack & 105 & 3 & 2 & 0.953 & 0.689\\ \hline
              ...
              一方で，IDF1はByteTrackを用いた提案手法のほうが大幅に高くなった．これは，人物同士が接近したり遮蔽が発生した際に，これまでと違うIDを付与してしまうIDスイッチがほとんど発生しなかったためであると考えられる．カルマンフィルタを用いて追跡を行う従来の手法では，多くの場面でIDスイッチが発生しており，持続性の高い追跡にはなっていない．
       ...
       本節では，骨格推定を含めた統合システムの評価を行う.
              ...
              まず，骨格推定モデルを学習する際に学習データとして用いた，シンプルな直進歩行に対して統合システムを実行して骨格を推定する．行う歩行は，2D-LiDARに向かって直進，2D-LiDARから離れる方向に直進，2D-LiDARの前を左右それぞれに向かって横断の4つである．ここでは，歩行しているように見えるかどうか定性的に判断すると同時に，明確に崩れたフレームの数を数えることで定量的な評価を行う．
              ...
              一方で，2D-LiDARに向かって直進する方向の歩行では推定精度が大きく下がった．歩行の向きとしては，前節の2D-LiDARに向かって直進する場合に近い．しかし，2D-LiDARから見て正面ではなく斜め前での歩行であったため，時系列画像に残る足の形が正面から見た形と変化してしまい，推定精度に影響したと考えられる．

              %正面と斜めのMHI並べる

              また，直進ではなく曲線的な動きをメインとする歩行では，ほとんど骨格の推定を行うことができなかった．これは，学習データに直線的な歩行しかないためであると考える．
